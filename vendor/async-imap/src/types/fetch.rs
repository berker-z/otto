...
impl Fetch {
    ...
    /// Extract Gmail's stable message id (X-GM-MSGID), if present.
    pub fn gmail_msgid(&self) -> Option<u64> {
        if let Response::Fetch(_, attrs) = self.response.parsed() {
            attrs
                .iter()
                .find_map(|av| match av {
                    AttributeValue::GmailMsgId(v) => Some(*v),
                    _ => None,
                })
        } else {
            unreachable!()
    ...
    /// Extract Gmail's thread id (X-GM-THRID), if present.
    pub fn gmail_thrid(&self) -> Option<u64> {
        if let Response::Fetch(_, attrs) = self.response.parsed() {
            attrs
                .iter()
                .find_map(|av| match av {
                    AttributeValue::GmailThrId(v) => Some(*v),
                    _ => None,
                })
        } else {
            unreachable!()
    ...
    /// Extract Gmail labels (X-GM-LABELS), if present.
    pub fn gmail_labels(&self) -> Vec<String> {
        if let Response::Fetch(_, attrs) = self.response.parsed() {
            attrs
                .iter()
                .find_map(|av| match av {
                    AttributeValue::GmailLabels(v) => Some(
                        v.iter()
