...
fn mailbox_data_search(i: &[u8]) -> IResult<&[u8], MailboxDatum<'_>> {
    // RFC 4551 suggests SEARCH responses may include a trailing "(MODSEQ <n>)" modifier:
    // "* SEARCH 2 5 6 7 (MODSEQ 917162500)"
    //
    // We currently ignore this modifier and only return the ids.
    let (i, ids) = preceded(tag_no_case(b"SEARCH"), many0(preceded(tag(" "), number)))(i)?;

    let (i, _) = opt(preceded(
        tag(" "),
        delimited(
            tag("("),
            tuple((tag_no_case("MODSEQ"), tag(" "), number_64)),
            tag(")"),
        ),
    ))(i)?;
